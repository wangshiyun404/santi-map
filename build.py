...
def inline(m):
    js = open(m.group(1).split('?')[0], encoding='utf8').read(); assert '</script' not in js, m.group(1); return '<script>\n' + js + '\n</script>'
body = re.sub(r'<script src="([^"]+)"></script>', inline, html)
body = re.sub(r'<link rel="stylesheet" href="css/style.css[^"]*">', lambda m: '<style>\n' + css + '\n</style>', body)
